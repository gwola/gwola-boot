...
import org.goodfox.gwola.util.excel.style.IExcelStyle;
import org.goodfox.gwola.util.excel.style.TemplateExcelStyle;
import org.goodfox.gwola.util.exception.GwolaBaseException;
import org.goodfox.gwola.util.utils.CollectionUtils;
import org.goodfox.gwola.util.utils.ObjectUtils;
    ...
    protected void addDataSelect() {
        for (ExcelFieldRule excelFieldRule : excelClassRule.getFieldRuleList()) {
            if (StringUtils.isNotEmpty(excelFieldRule.getDictType())) {
                ...
                if (labelArray == null) {
                    throw new GwolaBaseException("属性[" + excelFieldRule.getFieldName() + "],字典类型[" + excelFieldRule.getDictType() + "]对应的数据不存在");
                }
